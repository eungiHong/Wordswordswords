...
// 제약조건: 1. 블록의 길이는 그리드의 길이를 넘어선 안된다. 2. 블록의 길이는 가로 세로 중 어느 것 하나는 반드시 1이어야 하고, 나머지는 2 이상이어야 한다(홀로 고립되어 있는 블록이 있어선 안되며(가로 세로의 길이가 1인 경우), 가로 세로의 길이가 모두 2 이상이면, 단어를 삽입할 때 복잡도가 크게 증가하기 때문에).
